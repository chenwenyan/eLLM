...
template <typename scalar_t>
void hfused_add_rms_norm_impl(scalar_t* __restrict__ last_input,
                             scalar_t* __restrict__ input,
                             scalar_t* __restrict__ last_residual,
                             scalar_t* __restrict__ residual,
                             const scalar_t* __restrict__ weight,
                             const float epsilon, const int num_tokens,
                             const int hidden_size) {
...
void hfused_add_rms_norm(torch::Tensor& last_input, torch::Tensor& input,
                        torch::Tensor& last_residual, torch::Tensor& residual,
                        torch::Tensor& weight, float epsilon) {
  int hidden_size = input.size(-1);
  int num_tokens = input.numel() / hidden_size;

  VLLM_DISPATCH_FLOATING_TYPES(
      input.scalar_type(), "hfused_add_rms_norm_impl", [&] {
        CPU_KERNEL_GUARD_IN(hfused_add_rms_norm_impl)
        hfused_add_rms_norm_impl(
            input.data_ptr<scalar_t>(),
            last_residual.data_ptr<scalar_t>(), residual.data_ptr<scalar_t>(),
            weight.data_ptr<scalar_t>(), epsilon, num_tokens, hidden_size);
        CPU_KERNEL_GUARD_OUT(hfused_add_rms_norm_impl)
      });
}
