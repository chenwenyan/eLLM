 ...
 */
 ...
 __global__ void dotProductKernel(float* Q, float* K, float* QK, int batch_size, int seq_length, int d_model) {
  ...
  if (row < batch_size * seq_length && col < seq_length) {
      float sum = 0.0f;
      for (int i = 0; i < d_model; i++) {
          sum += Q[row * d_model + i] * K[col * d_model + i];
      }
      QK[row * seq_length + col] = sum / sqrtf(d_model);
  }
}

__global__ void softmaxKernel(float* QK, int batch_size, int seq_length) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (row < batch_size * seq_length && col < seq_length) {
      ...
      for (int i = 0; i < seq_length; i++) {
          if (QK[row * seq_length + i] > max_val) {
              max_val = QK[row * seq_length + i];
          }
      }

      float sum = 0.0f;
      for (int i = 0; i < seq_length; i++) {
          QK[row * seq_length + i] = expf(QK[row * seq_length + i] - max_val);
          sum += QK[row * seq_length + i];
      }

      for (int i = 0; i < seq_length; i++) {
          QK[row * seq_length + i] /= sum;
      }
  }
}

__global__ void attentionKernel(float* QK, float* V, float* output, int batch_size, int seq_length, int d_model) {
  int row = blockIdx.y * blockDim.y + threadIdx.y;
  int col = blockIdx.x * blockDim.x + threadIdx.x;

  if (row < batch_size * seq_length && col < d_model) {
      float sum = 0.0f;
      for (int i = 0; i < seq_length; i++) {
          sum += QK[row * seq_length + i] * V[i * d_model + col];
      }
      output[row * d_model + col] = sum;
  }
}

void attention(float* Q, float* K, float* V, float* output, int batch_size, int seq_length, int d_model) {
  float *d_Q, *d_K, *d_V, *d_QK, *d_output;
  size_t size = batch_size * seq_length * d_model * sizeof(float);
  size_t QK_size = batch_size * seq_length * seq_length * sizeof(float);
  ...
  attentionKernel<<<dimGrid, dimBlock>>>(d_QK, d_V, d_output, batch_size, seq_length, d_model);
...
__global__ void layer_norm_kernel(float* input, float* output, float* gamma, float* beta, int n, int d, float epsilon) {
  ...
  // Normalize and scale
  for (int i = 0; i < d; ++i) {
      float normalized = (input[idx * d + i] - mean) / sqrtf(var + epsilon);
...
__global__ void mlp_kernel(float* input, float* output, float* weights1, float* biases1, float* weights2, float* biases2, int n, int d_in, int d_hidden, int d_out) {
  ...
  // First layer
  float hidden[1024]; // Assuming d_hidden <= 1024
  for (int i = 0; i < d_hidden; ++i) {
      hidden[i] = 0.0f;
      for (int j = 0; j < d_in; ++j) {
          hidden[i] += input[idx * d_in + j] * weights1[j * d_hidden + i];
      }
      hidden[i] += biases1[i];
  ...
  for (int i = 0; i < d_out; ++i) {
      output[idx * d_out + i] = 0.0f;
      for (int j = 0; j < d_hidden; ++j) {
          output[idx * d_out + i] += hidden[j] * weights2[j * d_out + i];
      }
      output[idx * d_out + i] += biases2[i];
  }
}
...
void mlp(float* input, float* output, float* weights1, float* biases1, float* weights2, float* biases2, int n, int d_in, int d_hidden, int d_out) {
 ...
 template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE,
           int NUM_THREADS, vllm::Fp8KVCacheDataType KV_DTYPE,
           int PARTITION_SIZE = 0>  // Zero means no partitioning.
 __device__ void fused_paged_attention_kernel(
     float* __restrict__ exp_sums,  // [num_seqs, num_heads, max_num_partitions]
     float* __restrict__ max_logits,  // [num_seqs, num_heads,
                                      // max_num_partitions]
 ...
 template <typename scalar_t, typename cache_t, int HEAD_SIZE, int BLOCK_SIZE,
           int NUM_THREADS,
           vllm::Fp8KVCacheDataType KV_DTYPE>
 __global__ void fused_paged_attention_v1_kernel(
     scalar_t* __restrict__ last_out,           // [num_seqs, num_heads, head_size]
     const scalar_t* __restrict__ last_q,       // [num_seqs, num_heads, head_size]  
     scalar_t* __restrict__ out,           // [num_seqs, num_heads, head_size]
     const scalar_t* __restrict__ q,       // [num_seqs, num_heads, head_size]
     ...
     const int q_stride, const int kv_block_stride, const int kv_head_stride,
     const float kv_scale) {
   fused_paged_attention_kernel<scalar_t, cache_t, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS,
                          KV_DTYPE>(
 ...
 #define LAUNCH_FUSED_PAGED_ATTENTION_V1(HEAD_SIZE)                         \
 VLLM_DevFuncAttribute_SET_MaxDynamicSharedMemorySize(                     \
     ((void*)vllm::fused_paged_attention_v1_kernel<                              \
         T, CACHE_T, HEAD_SIZE, BLOCK_SIZE, NUM_THREADS, KV_DTYPE>),       \
     shared_mem_size);                                                     \
 vllm::fused_paged_attention_v1_kernel<T, CACHE_T, HEAD_SIZE, BLOCK_SIZE,        \
                                 NUM_THREADS, KV_DTYPE>                    \
     <<<grid, block, shared_mem_size, stream>>>(                           \
         last_out_ptr, last_q_ptr, out_ptr, query_ptr, key_cache_ptr, value_cache_ptr, num_kv_heads, \
...
// TODO(woosuk): Tune NUM_THREADS.
  ...
  vllm::Fp8KVCacheDataType KV_DTYPE, int NUM_THREADS = 256>
...
// 1, 2, 4, 64, 128, 256.
 ...
 void fused_paged_attention_v1(
  ...
  torch::Tensor& last_q,       // [num_seqs, num_heads, head_size]
