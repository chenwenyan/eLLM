...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if residual is None:
            ...
            # print("residual is None, hidden_states is ", hidden_states)
            ...
            for i in range(len(self.layers)-1):
                if i <= int(len(self.layers)*self.cache_config.store_cache_layers)-3:
                    ...
                    with cuda.stream(stream1):
                        hidden_states1, residual1 = self.layers[i+1](
                            self.last_positions,
                            self.last_hidden_states,
                            kv_caches[int(len(self.layers)*self.cache_config.store_cache_layers)-2],
                            self.last_attn_metadata,
                            self.last_residual,
                        )
                        self.last_hidden_states = hidden_states1
                        self.last_residual = residual1

                    # 15层的token_10 decoding阶段    
