...
LOG_DIR=logs/1_colorun_llm_1gpu/colo_swap_recompute_varied_request_rate
...
CUDA_VISIBLE_DEVICES=${gpu_id} python3 -m vllm.entrypoints.openai.api_server \
    ...
    --disable-log-requests > ${LOG_DIR}/${model1_name}_server_${gpu_memory_utilization}_${request_rate}_${num_prompt}_${preemption_mode1}_${preemption_mode2}.log & 
pid1=$!
sleep 10
...
CUDA_VISIBLE_DEVICES=${gpu_id} python3 -m vllm.entrypoints.openai.api_server \
    ...
    --disable-log-requests > ${LOG_DIR}/${model2_name}_server_${gpu_memory_utilization_2}_${request_rate}_${num_prompt}_${preemption_mode2}_${preemption_mode1}.log & 
pid2=$!    
sleep 10
...
for num_prompt in ${num_prompts[@]}; do
    for request_rate in ${request_rates[@]}; do
        python3 ../benchmarks/benchmark_serving.py \
            ...
            --endpoint /v1/completions > ${LOG_DIR}/${model1_name}_client_${gpu_memory_utilization}_${request_rate}_${num_prompt}_${preemption_mode1}_${preemption_mode2}.log &
        pid3=$!    
        ...
        python3 ../benchmarks/benchmark_serving.py \
            ...
            --num-prompts ${num_prompt} \
            --result-dir results/1_colorun_llm_1gpu \
            --endpoint /v1/completions > ${LOG_DIR}/${model2_name}_client_${gpu_memory_utilization_2}_${request_rate}_${num_prompt}_${preemption_mode2}_${preemption_mode1}.log &
        pid4=$!    
