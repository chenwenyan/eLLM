            ...
            for num_prompt in ${num_prompts[@]}; do
                for store_cache_layers in ${store_cache_layerss[@]}; do
                    for wt_weight in ${wt_weights[@]}; do
                        CUDA_VISIBLE_DEVICES=${gpu_id} taskset -c 6-7 python3 -m vllm.entrypoints.openai.api_server \
                            --model ${model} \
                            --port 8081 \
                            ...
                            --disable-log-requests > ${path_dir}/${model_name}_server_${gpu_memory_utilization}_${request_rate}_${num_prompt}_${preemption_mode}_${store_cache_layers}_${scheduling_policy}_wt${wt_weight}.log & 
                        # > server.log 2>&1 &
                        pid=$!    
                        ...
                        python3 ../benchmarks/benchmark_serving.py \
                            ...
                            --endpoint /v1/completions  >> ${path_dir}/${model_name}_client_${gpu_memory_utilization}_${request_rate}_${num_prompt}_${preemption_mode}_${store_cache_layers}_${scheduling_policy}_wt${wt_weight}.log    
                        # > client.log     
                        kill -9 $pid 
                        sleep 1
                    done
